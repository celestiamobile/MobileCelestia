...
struct GuideView: UIViewControllerRepresentable {
    ...
    @Environment(RequestHandlerWrapper.self) private var requestHandlerWrapper

    let id: String
    let resourceManager: ResourceManager

    func makeUIViewController(context: Context) -> CommonWebViewController {
        return CommonWebViewController(executor: renderer, resourceManager: resourceManager, url: .fromGuide(guideItemID: id, language: AppCore.language), requestHandler: requestHandlerWrapper.requestHandler, actionHandler: { action, viewController in
