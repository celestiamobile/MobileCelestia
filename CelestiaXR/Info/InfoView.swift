...
struct InfoView: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> InfoViewController {
        let vc = InfoViewController(configuration: InfoViewController.Configuration(info: selection, core: renderer.appCore, showNavigationTitle: isEmbeddedInNavigationController, backgroundColor: nil))
