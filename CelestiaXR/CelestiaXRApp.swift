...
@Observable class RequestHandlerWrapper {
    let requestHandler: RequestHandlerImpl

    init(requestHandler: RequestHandlerImpl) {
...
struct CelestiaXRApp: App {
    ...
    var body: some Scene {
        ...
        WindowGroup(id: "Tool") {
            ToolView(userDefault: userDefaults, bundle: bundle, defaultDataDirectory: defaultDataDirectoryURL, defaultConfigFile: defaultConfigFileURL, userDirectory: userDirectory, resourceManager: resourceManager, requestHandler: requestHandler)
        ...
        WindowGroup(id: "GuideWindow", for: String.self) { $id in
            GuideView(id: id ?? "", resourceManager: resourceManager)
                .disabled(renderer.rendererStatus != .rendering)
                .environment(renderer)
        }
        .defaultSize(Constants.windowSize)
