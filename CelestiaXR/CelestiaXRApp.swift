...
struct CelestiaXRApp: App {
    ...
    var body: some Scene {
        ...
        WindowGroup(id: "Tool") {
            ToolView(userDefault: userDefaults, bundle: bundle, defaultDataDirectory: defaultDataDirectoryURL, defaultConfigFile: defaultConfigFileURL, userDirectory: userDirectory, resourceManager: resourceManager)
        ...
        WindowGroup(id: "AddonWindow", for: String.self) { $id in
            AddonWindow(id: id ?? "", resourceManager: resourceManager)
                .disabled(renderer.rendererStatus != .rendering)
                .environment(renderer)
        }
        .defaultSize(Constants.windowSize)
